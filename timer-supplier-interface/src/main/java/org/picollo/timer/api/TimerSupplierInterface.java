...
public abstract class TimerSupplierInterface extends AbstractConfigurableInterface {
    ...
    private CompletableFuture<Scheduler> schedulerFuture = new CompletableFuture<>();
...

    {
        try {
            ...
            scheduler.start();
            schedulerFuture.complete(scheduler);
    ...
    protected void setupTrigger() {
        ...
        if( getJobClass() == null )
            return;
        try {
            // blocks until don't have a scheduler
            scheduler = schedulerFuture.get();
            removeJob();
